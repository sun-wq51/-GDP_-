...
wmw = pygal_maps_world.maps.World()  #导入需要使用的库
...
def reconcile_countries_by_name(plot_countries, gdp_countries): #返回在世行有GDP数据的绘图库国家代码字典，以及没有世行GDP数据的国家代码集合
    ...
    for pygal_country_code in plot_countries :
        if plot_countries[pygal_country_code] in gdp_countries : 
            dict1[pygal_country_code] = plot_countries[pygal_country_code]   # 字典内容为在世行数据里的绘图库国家（键为绘图库各国家代码，值为对应的具体国名)

        else :
            set1.add(pygal_country_code)   #  集合内容为不在世行数据的绘图库国家代码
    ...
    for plot_countries_code in plot_countries :
        for isp_country_code in gdpinfo :
            ...
            if gdpinfo[isp_country_code]["Country Name"] == plot_countries[plot_countries_code] : # 判断绘图库里的国家在不在世界银行数据里
                ...
                for country_information in gdpinfo[isp_country_code] :
                    country_informations.append(country_information)
                
                for year1 in country_informations[4:-1] :
                    gdp_number += gdpinfo[isp_country_code][year1]
                    
                if gdp_number == '' :
                    set2.add(plot_countries_code)  # 把该国家归为不在世界银行数据里
                    ...
                    if gdpinfo[isp_country_code][year] != '' :
                        ...
                        set3[plot_countries_code] = '该年暂无数据'  # 把该国家分类为该年暂无数据
...
def render_world_map(gdpinfo, plot_countries, year, map_file): #将具体某年世界各国的GDP数据(包括缺少GDP数据以及只是在该年缺少GDP数据的国家)以地图形式可视化
    ...
    wmw.title = '世界银行%s年国家GDP数据'%(year)    #  添加图片标题
    wmw.add('该年在绘图库及世行有数据的国家及其GDP数据', A_countries)
    wmw.add('在绘图库数据里没有的国家',B_countries)
    wmw.add('该年在世行没有GDP数据的国家',C_countries)
    
    wmw.render()

    wmw.render_to_file(map_file)   # 输出生成 .scv文件
